...
public class BillResourceIT {
	...
	public void testCreateSelfLinkDelete() {
		...
		Response getBasketResponse = client.target(createBasketResponse.getLocation()).request().get();
		...
		for (Link link : getArticleResponse.getLinks()) {
		...
		Response getBillSelfResponse = client.target(getBillResponse.getLink("self")).request().get();
		Assert.assertEquals(Status.OK.getStatusCode(), getBillSelfResponse.getStatus());
	...
	public void testPaging() {
		...
		Collection<Bill> page = secondPage.readEntity(new GenericType<Collection<Bill>>() {
		});
