...
public class ArticleResourceIT {
	...
	public void testSelfLink() {
		Article newArticle = new Article("Created", "created...", new BigDecimal(13), null);
	...
	public void testPaging() {
		...
		for (int i = 0; i < 100; i++) {
			Article newArticle = new Article("Created" + i, "created..." + i, new BigDecimal(i), null);
			Response response = articleTarget.request().post(Entity.entity(newArticle, MediaType.APPLICATION_JSON));
			Assert.assertTrue(response.getStatus() == Status.CREATED.getStatusCode());
		...
		Collection<Article> page = secondPage.readEntity(new GenericType<Collection<Article>>() {
		});
		...
		Assert.assertEquals("Created99", nextResponse.readEntity(new GenericType<List<Article>>() {
		}).get(9).getName());
	...
	public void testAddToBasket() {
		Article newArticle = new Article("Article for basket", "f00", new BigDecimal(13), null);
		...
		Assert.assertTrue(getResponse.getLinks().size() > 1);

		Link addToBasketLink = null;
		for (Link link : getResponse.getLinks()) {
			if (link.getRel().contains("tos:addToBasket")) {
				addToBasketLink = link;
			}
		}
